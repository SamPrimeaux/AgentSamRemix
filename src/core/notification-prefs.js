...
/** Flat keys used by the settings UI (notify.<event>). */
...
export function normalizeNotifyBag(raw) {
  ...
  const webhookUrl = trim(base.webhook_url);
  return {
    events,
...
/**
 * Flat map for dashboard toggles: { 'notify.deploy_success': 'true'|'false', 'notify.webhook_url': '...' }
...
export function notifyBagToFlat(bag) {
  ...
  flat['notify.webhook_url'] = bag.webhook_url || '';
  return flat;
}
...
export function applyFlatNotifyUpdates(current, updates) {
  ...
  for (const u of list) {
    ...
    if (key === 'notify.webhook_url') {
      next.webhook_url = trim(val);
      continue;
    }
