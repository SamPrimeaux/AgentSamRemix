/**
 * User-scoped Web Push delivery.
 * Subscription records remain in agentsam_hook; preference state remains in user_settings.
 */
import { sendNotification } from 'web-push-neo';
import { readNotificationPrefs } from './notification-prefs.js';

function configured(env) {
  return Boolean(env?.VAPID_PUBLIC_KEY && env?.VAPID_PRIVATE_KEY && env?.VAPID_SUBJECT);
}

export async function sendIdentityWebPushToUser(env, payload = {}) {
  ...
  for (const row of rows.results || []) {
    ...
    try {
      subscription =
        typeof row.handler_config === 'string'
    ...
    try {
      ...
      /* A channel failure must not fail the originating notification. */
    }
  }
  return { ok: true, sent };
}
