...
/**
 * Resolve a stored OAuth access token without provider routing.
 * Kept in this leaf so integrations never import the HTTP OAuth controller.
 */
export async function resolveOAuthAccessToken(env, row) {
  ...
  if (row.access_token_encrypted) {
    try {
      ...
      console.warn('[user-oauth-token] decrypt access_token failed:', e?.message);
    }
  }
  return row.access_token || null;
}

/** Resolve a stored OAuth refresh token without provider routing. */
export async function resolveOAuthRefreshToken(env, row) {
  ...
  if (row.refresh_token_encrypted) {
    try {
      ...
      console.warn('[user-oauth-token] decrypt refresh_token failed:', e?.message);
    }
  }
...
/**
 * Supabase Management API OAuth credentials only (https://api.supabase.com).
 * These are distinct from project Auth/login OAuth credentials.
 */
export function getSupabaseManagementOAuthCredentials(env) {
  const id =
...
/** Supabase Management OAuth is account-wide; workspace is only a selection hint. */
export function supabaseOAuthAccountIdentifier(workspaceId) {
  void workspaceId;
  return 'Supabase';
}

/** Live Supabase Management project catalog for a bearer token. */
export async function fetchSupabaseManagementProjects(accessToken) {
 ...
 * Prefers account_identifier `Supabase`, then legacy workspace:* rows.
 * workspaceId is a preference/metadata hint only — never an authorization gate.
 */
export async function getUserSupabaseToken(env, userId, workspaceId = null) {
  ...
  let access = await resolveOAuthAccessToken(env, fullRow);
  let refresh = await resolveOAuthRefreshToken(env, fullRow);

  const exp = Number(fullRow.expires_at);
  const mgmt = getSupabaseManagementOAuthCredentials(env);
  const needsRefresh =
    refresh && mgmt && (!Number.isFinite(exp) || exp <= nowSeconds() + 300);
  ...
  if (needsRefresh) {
    ...
    if (tok?.access_token) {
      ...
      fullRow = { ...fullRow, metadata_json, expires_at: newExp };
    }
  }
  ...
  return {
    ...
    metadata: meta,
    account_identifier:
      fullRow.account_identifier != null ? String(fullRow.account_identifier) : accountKey,
  };
}
