...
import { tableExists, safeQueryAll, safeFirst, envelope } from './shared.js';
...
async function putIndexRules(request, env, authUser) {
  ...
  const userId = (await resolveCanonicalUserId(String(authUser?.id || '').trim(), env)) || String(authUser?.id || '').trim();
